...
if ( $userlogged == 1 && $gotusrid =~ /54LD/ ) {
   print "<p>Привет волонтёр $b1 $userFirstName{$gotusrid}!$b2 <a href = \'$indexcgi?action=logout\'>Logout</a></p>\n";
   print "<p> </p>\n";
########################## START ADMIN PART #########################################
...
######################### FINISH ADMIN PART #########################################
   if ( $userGroupId{$gotusrid} =~ /M/ ) { &openBlankPage(); }
   else { print "<p> </p>$rf<h3> Нет Данных для Показа! </h3>$f2<p> </p>\n" ; }
}
else{
...
sub openBlankPage {
   ...
   $bpsStartDayNum = $calendarnumber{$bpsStartDate}; $bpsFinishDayNum = $calendarnumber{$bpsFinishDate};  $bpsCnt = 1; $globalCont = 0 ;
   print "<h1>Список всех уроков по группе [$userGroupId{$gotusrid}] на дату:</h1>
   ...
   $whatis{'groupid'} = $userGroupId{$gotusrid}; &openGroupSub($bpsStartDayNum, $bpsFinishDayNum, $bpsCnt); 
   print "</table><p> </p><p><a href=\'$httpactivecgi\'>Вернуться ДОМОЙ</a></p><p> </p>\n"; 
}
